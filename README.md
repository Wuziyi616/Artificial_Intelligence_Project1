...
The code is formatted as follows: UI.ui is the file developed using QtDesigner and ui.py is generated by PyQt5; the folder normal_tangram contains code for solving the 7 elements puzzle, the folder thirteen_tangram contains code for solving the 13 elements puzzle, and the folder any_tangram contains code for any input image puzzle. Finally tangram_solver.py summarize all the above codes and provide you with a user interface.  
More specifically, if you click into one of the code folders, you will generally see 5 kind of .py file:  
- config.py is about some hyperparameters and setting for the puzzle (like number of elements)
- image_utils.py is some utility functions processing images (like read in a gray scale image)
- utils.py is some functions handling general problem (like calculating distance between two points)
- tangram_element.py is the definition for elements used in the puzzle (like Triangle)
- search_algorithm.py is my core implementation of searching (DFS)
